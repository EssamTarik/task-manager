...
class ProjectReports extends Component{
	...
	getTotalTime(){
		if(this.state.selected === 0 ){
			return this.props.Project.totalTime;
		}else{
			let total = 0;
	...
	getChartData(){
		let start = this.state.selected !== 0 ? new moment(new Date(this.getCurrentSprint().startDate)) : new moment(new Date(this.props.Project.startDate));
		...
		let end = new moment(new Date(this.getMaxDate()));
		...
		let OptimalWorkDays = Math.ceil(optimalDuration.asDays());
		let workDays = Math.ceil(duration.asDays());
		
		let points = [{x: 0, y: totalWorkHours}];
		let dayms = 86400000;
		...
		for(let x = 1; x<=workDays; x++){
			...
			let timeBetween = this.getFinishedBetween((x-1)*dayms+startDateObj.getTime(), x*dayms+startDateObj.getTime());
			totalFinished += timeBetween;
			let remaining = totalWorkHours - totalFinished;
			if(timeBetween === 0)
				remaining = lastRemaining;
			points.push({x: x, y: remaining});
