...
class Event extends Component{
	render(){
		this.color = "white";
		if(this.props.row.event === "todo"){
			this.backgroundColor = "#039BE5";
			this.status = "To Do"
		}
		else if(this.props.row.event === "done"){
			this.backgroundColor = "#43A047";
			this.status = "Done"
		}
		else if(this.props.row.event === 'inprogress'){
			this.backgroundColor = "#FFEB3B";
			this.color = '#424242';
			this.status = "In Progress"
		}else if(this.props.row.event === 'created'){
			this.backgroundColor = "#5C6BC0";
			this.status = 'Created'
		}else if(this.props.row.event === 'edited'){
			this.backgroundColor = "#37474F";
			this.status = 'Edited'
		}

		return (
			<div className="uk-border-rounded" style={{width: '60%', textAlign: 'center', color: this.color, backgroundColor: this.backgroundColor}}>{this.status}</div>
		);
	}
}
...
export {Event, DateView, CompleteProgressBar, URL, remainingDays, SprintProgressBar, UserImage, TaskTypeIcon};